...
const member = {
    ...
    join : async function(data) {
        try{
            ...
            // data는 문자열로만 기입 가능
            // JSON      .parse 문자 형태의 JSON을 실제자바스크립트 객체 변환 / .stringify 자바스크립트 객체 > 문자열 형태의 JSON 데이터로 변환
    ...
    // 로그인 처리
    // 회원 아이디, 비번
    //      1. 회원 아이디를 통해서 가입된 회원 정보를 조회
    //      2. 회원 정보의 비밀번호 해시와 입력된 비밀번호를 비교 
    //      3. 일치하면 세션에 비밀번호가 일치하는 회원의 아이디를 저장 > req.session의 하위 속성으로 등록 (req 객체 > 세션을 사용하기 위해)
    //      4. 전역 적용 미들웨어 세션에 저장된 아이디를 통해 회원정보를 조회 > 전역에 유지
    login : async function (memId, memPw, req) {
        try {
            ...
            // 회원정보가 없거나, 비밀번호가 일치 하지 않으면 로그인 실패
            return false;
        }
    }, 
    ...
    get : async function(memId) {
        ...
        try {
            if(!memId)
                // 아이디가 빈값, undefined인 경우
                return false;

            const filePath = path.join(__dirname, "../data/member", memId + ".json");
            let data = await fs.readFile(filePath); // 버퍼 데이터형태 toString();
