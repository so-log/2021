...
// 회원 가입 유효성 검사 미들웨어
module.exports.joinVaildator = async (req, res, next) => {
    // console.log("미들웨어", req.body);
    // 입력 필수 데이터 : memId, memPw, memPwRe, memNm
    // 객체의 반복문 : for 속성명 in 객체
    const required = {
        memId : "아이디를 입력하세요.",
        memPw : "비밀번호를 입력하세요.",
...
module.exports.loginValidator = (req, res, next) => {
    try {
        ...
        return alert(err.massage, res, true);
    }
    next();
};